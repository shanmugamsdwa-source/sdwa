...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onCategoryCreated: (category: AchievementCategory) => void;
}

export default function InlineCategoryModal({ isOpen, onClose, onCategoryCreated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newId = await createDocument(COLLECTIONS.ACHIEVEMENT_CATEGORIES, payload);
      onCategoryCreated({
        id: newId,
        name: name.trim(),
        slug: name.trim().toLowerCase().replace(/\s+/g, '-'),
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1E293B] border border-slate-700 rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-700">
          <h3 className="font-bold text-white text-base">New Achievement Category</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Masters Men, Youth Girls"
