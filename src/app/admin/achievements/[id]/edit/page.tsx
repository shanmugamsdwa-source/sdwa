...
export default function EditAchievementPage({
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [aData, cats, levs] = await Promise.all([
        getDocument<Achievement>(COLLECTIONS.ACHIEVEMENTS, id),
      ...
      if (aData) {
        setFormData({
          ...
          levelId: aData.levelId || '',
          startYear: aData.startYear || (aData.season ? parseInt(aData.season.split('/')[0]) : 2025),
          endYear: aData.endYear || (aData.season ? parseInt(aData.season.split('/')[1]) : 2026),
          venue: aData.venue || '',
          description: aData.description || '',
          displayOrder: aData.displayOrder || 1,
          isFeatured: aData.isFeatured ?? false,
          isPublished: aData.isPublished ?? true,
        });

        if (aData.images) {
          setImages(
            aData.images.map((img, idx) => ({
              url: img.url,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDocument(COLLECTIONS.ACHIEVEMENTS, id, payload);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12 text-slate-400">
        <Loader2 className="animate-spin mr-2" /> Loading achievement...
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Update championship award or details</p>
        </div>
      </div>
