...
export default function AchievementLevelsAdminPage() {
  const [levels, setLevels] = useState<AchievementLevel[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingLevel, setEditingLevel] = useState<Partial<AchievementLevel> | null>(null);
  ...
  const fetchLevels = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getCollection<AchievementLevel>(
      ...
      console.error(err);
      setError('Failed to load achievement levels');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        slug: (editingLevel.name || '').toLowerCase().replace(/\s+/g, '-'),
        displayOrder: editingLevel.displayOrder ?? levels.length + 1,
        isActive: editingLevel.isActive ?? true,
      };

      if (isNew) {
        await createDocument(COLLECTIONS.ACHIEVEMENT_LEVELS, payload);
      } else if (editingLevel.id) {
        await updateDocument(COLLECTIONS.ACHIEVEMENT_LEVELS, editingLevel.id, payload);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDocument(COLLECTIONS.ACHIEVEMENT_LEVELS, id);
      fetchLevels();
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Achievement Levels</h1>
            <p className="text-slate-400 text-sm mt-0.5">
              Manage competition tiers (District, Zonal, State, National, International)
            </p>
          </div>
        </div>
        <button
          onClick={() => {
            setEditingLevel({ displayOrder: levels.length + 1, isActive: true });
            setIsNew(true);
          }}
          ...
          <div className="bg-[#1e293b] border border-slate-700 rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={editingLevel.displayOrder ?? 1}
                  onChange={(e) => setEditingLevel({ ...editingLevel, displayOrder: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 bg-[#0f172a] border border-slate-600 rounded-lg text-white text-sm"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                <input
                  type="checkbox"
                  id="isActiveLev"
                  checked={editingLevel.isActive ?? true}
                ...
                />
                <label htmlFor="isActiveLev" className="text-sm text-slate-300">
                  Active
                </label>
              </div>
      ...
      <div className="bg-[#1e293b] border border-slate-800 rounded-2xl overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-400">Loading levels...</div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-800">
              {levels.map((l) => (
                <tr key={l.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs text-slate-400">{l.displayOrder}</td>
                  <td className="px-6 py-4 font-semibold text-white">{l.name}</td>
                  <td className="px-6 py-4 font-mono text-xs text-slate-400">{l.slug}</td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        l.isActive ? 'bg-green-500/10 text-green-400' : 'bg-slate-700 text-slate-400'
                      }`}
                    >
